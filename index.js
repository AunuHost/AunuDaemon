/**
 * @fileoverview Main entry file for the Aunu Daemon. This module sets up an
    ...
    wss.on('connection', (ws, req) => {
        ...
        function authenticateWebSocket(ws, req, password, callback) {
            if (password === config.key) {
                ...
                ws.send(`\r\n\u001b[33m[AunuDaemon] \x1b[0mconnected!\r\n`);
                const urlParts = req.url.split('/');
                const containerId = urlParts[2];
        ...
        async function performPowerAction(ws, container, action) {
            ...
            try {
                ...
                const successMessage = {
                    timestamp: new Date().toISOString(),
                    content: `\r\n\u001b[32m[AunuDaemon] \x1b[0m${action.charAt(0).toUpperCase() + action.slice(1)} action completed.\r\n`,
                };
                ...
                const errorMessage = {
                    timestamp: new Date().toISOString(),
                    content: `\r\n\u001b[31m[AunuDaemon] \x1b[0mAction failed: ${err.message}\r\n`,
                };
...
app.get('/', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Docker is not running - AunuDaemon will not function properly.' });
    }
});
 ...
 */
setTimeout(function (){
  server.listen(config.port, () => log.info(`AunuDaemon is listening on port ${config.port}`));
}, 2000);